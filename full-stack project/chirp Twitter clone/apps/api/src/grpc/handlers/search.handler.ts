...
import { runGrpcHandler } from "../handler-utils";
...
export const searchHandler: ISearchService = {
	async searchPosts(request) {
		return runGrpcHandler({ service: "SearchService", method: "searchPosts" }, async () => {
	...
	async searchUsers(request) {
		return runGrpcHandler({ service: "SearchService", method: "searchUsers" }, async () => {
