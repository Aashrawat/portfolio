...
export async function getPost(postId: string, userId?: string) {
	...
	const [enriched] = await enrichPostsWithCounts([post], userId);
	return enriched;
}
...
export async function getPosts(options: GetPostsOptions = {}) {
	...
	return enrichPostsWithCounts(result, options.userId);
}

export async function getUserPosts(username: string, userId?: string, limit = 20, offset = 0) {
